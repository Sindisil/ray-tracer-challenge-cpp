...
TEST_CASE("Constructing and inspecting a 4x4 matrix") {
  Mat4 m{1.f, 2.f,  3.f,  4.f,  5.5f,  6.5f,  7.5f,  8.5f,
...
TEST_CASE("Matrix equality with different matrices") {
  Mat4 a{1, 2, 3, 4, 5, 6, 7, 8, 9.1f, 8.1f, 7.1f, 6.1f, 5, 4, 3, 2};
  Mat4 b{1, 2, 3, 4, 5, 6, 7, 8, 9.1f, 8.1f, 7.1f, 6.1f, 5, 4, 3, 2.1f};
  Mat4 c{1, 2, 3, 4, 5, 6, 7, 0, 9.1f, 8.1f, 7.1f, 6.1f, 5, 4, 3, 2.0f};
  CHECK(a != b);
  CHECK(a != c);
}
...
TEST_CASE("Multiplying two matrices") {
  ...
  Mat4 c{20, 22, 50, 48, 44, 54, 114, 108, 40, 58, 110, 102, 16, 26, 46, 42};
...
TEST_CASE("Multiplying a matrix by the identity matrix") {
  Mat4 m{0, 1, 2, 4, 1, 2, 4, 8, 2, 4, 8, 16, 4, 8, 16, 32};
...
TEST_CASE("Transposing a matrix") {
  ...
  Mat4 transposed{0, 9, 1, 0, 9, 8, 8, 0, 3, 0, 5, 5, 0, 8, 3, 8};
...
TEST_CASE("Testing a noninvertable matrix for invertability") {
  Mat4 a{-4, 2, -2, 3, 9, 6, 2, 6, 0, -5, 1, -5, 0, 0, 0, 0};
...
TEST_CASE("Calculating the inverse of a matrix") {
  Mat4 a{-5, 2, 6, -8, 1, -5, 1, 8, 7, 7, -6, -7, 1, -3, 7, 4};
  auto b = a.invert();
  ...
  CHECK(b == Mat4{0.21805f, 0.45113f, 0.24060f, -0.04511f, -0.80827f, -1.45677f,
...
TEST_CASE("Calculating the inverse of another matrix") {
  Mat4 a{8, -5, 9, 2, 7, 5, 6, 1, -6, 0, 9, 6, -3, 0, -9, -4};
  CHECK(a.invert() == Mat4{-.15385f, -.15385f, -.28205f, -.53846f, -.07692f,
                           .12308f, .02564f, .03077f, .35897f, .35897f, .43590f,
                           .92308f, -.69231f, -.69231f, -.76923f, -1.92308f});
}

TEST_CASE("Multiplying a product by its inverse") {
  Mat4 a{{{3, -9, 7, 3}, {3, -8, 2, -9}, {-4, 4, 4, 4}, {1, -6, 5, -1}}};
