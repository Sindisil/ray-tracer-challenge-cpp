...
TEST_CASE("Multiplying two matrices") {
  ...
  Matrix<4> b{-2, 1, 2, 3, 3, 2, 1, -1, 4, 3, 6, 5, 1, 2, 7, 8};
  Matrix<4> c{20, 22, 50,  48,  44, 54, 114, 108,
...
TEST_CASE("Transposing a matrix") {
  Matrix<4> a{0, 9, 3, 0, 9, 8, 0, 8, 1, 8, 5, 3, 0, 0, 5, 8};
  Matrix<4> transposed{0, 9, 1, 0, 9, 8, 8, 0, 3, 0, 5, 5, 0, 8, 3, 8};
...
TEST_CASE("A submatrix of a 3x3 matrix is a 2x2 matrix") {
  Matrix<3> a{1, 5, 0, -3, 2, 7, 0, 6, -3};

  CHECK(a.submatrix(0, 2) == Matrix<2>{-3, 2, 0, 6});
}

TEST_CASE("A submatrix of a 4x4 matrix is a 3x3 matrix") {
  Matrix<4> a{-6, 1, 1, 6, -8, 5, 8, 6, -1, 0, 8, 2, -7, 1, -1, 1};
  CHECK(a.submatrix(2, 1) == Matrix<3>{-6, 1, 6, -8, 8, 6, -7, -1, 1});
}
