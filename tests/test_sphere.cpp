...
TEST_CASE("Comparing spheres") {
  auto s1 = Sphere{};
  auto s2 = Sphere{};
  auto s3 = Sphere{identity_matrix().scale(0.5, 0.5, 0.5)};
  auto s4 = s1;
  CHECK(s1 == s1);
  CHECK(s1 != s3);
  CHECK(!s1.is(s2));
...
TEST_CASE("The normal on a sphere at a nonaxial point") {
  Sphere s;
  auto n = s.normal_at(
      Point{std::sqrt(3.0) / 3.0, std::sqrt(3.0) / 3.0, std::sqrt(3.0) / 3.0});
  CHECK(n ==
        Vec3{std::sqrt(3.0) / 3.0, std::sqrt(3.0) / 3.0, std::sqrt(3.0) / 3.0});
}
...
TEST_CASE("Computing the normal on a translated sphere") {
  ...
  CHECK(n == Vec3{0, 0.70711, -0.70711});
}

TEST_CASE("Computing the normal on a transformed sphere") {
  Sphere s(identity_matrix().rotate_z(pi / 5.0).scale(1, 0.5, 1));
  auto n = s.normal_at(Point{0, std::sqrt(2.0) / 2, -std::sqrt(2.0) / 2});
  CHECK(n == Vec3{0, 0.97014, -0.24254});
}
...
TEST_CASE("A sphere may be created with a specified material") {
  auto m = Material{Color{1.0, 1.0, 1.0}};
  auto s = Sphere{m};
  CHECK(s.material() == m);
}
